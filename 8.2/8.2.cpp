...
int main()
{
	...
	do {
		...
		cout << " [3] - вивід кількості оцінок «відмінно»" << endl;
		...
		case 3:
			count = Ocinka(p, N);
			cout << "====================================" << endl;
			cout << "| Якщо оцінка >= 12, то це «відмінно» |" << endl;
			cout << "------------------------------------" << endl;
			cout << "| Кількість оцінок «відмінно»: " << setw(6) << count << " |" << endl;
			cout << "====================================" << endl;
			break;
		case 0:
			break;
...
int Ocinka(Student* p, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		case KOMPUTERNI_NAUKY:
			...
			if (p[i].matematika >= 12)
			{
				count++;
			}
			if (p[i].programming >= 12)
			{
				count++;
			}
			break;
